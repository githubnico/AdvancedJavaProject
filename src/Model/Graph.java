...
public class Graph {
    ...
    /**
     * Generates array from ArrayList<Pair<Integer,Integer>>
     *
     * @param myList ArrayList<Pair<Integer,Integer>>
     * @return
     */
    public static int[][] ArrayListToArray(ArrayList<Pair<Integer,Integer>>  myList){
        int[][] myArray = new int[myList.size()][2];
        for (int i = 0; i < myList.size(); i++) {
            myArray[i][0]  = myList.get(i).getKey();
            myArray[i][1] = myList.get(i).getValue();
        }
        return myArray;
    }
